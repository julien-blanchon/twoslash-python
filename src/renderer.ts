    ...
    handlers: {
      code: ((state, node) => {
        ...
        if (lang) {
          ...
          return (result.children?.[0] as Element) || defaultHandlers.code(state, node);
        }
 ...
 */
export function rendererRichPython(options: RendererRichOptionsPython = {}): TwoslashRenderer {
  const {
